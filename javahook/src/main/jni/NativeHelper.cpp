...
static void
initMethod(JNIEnv *env, jclass clazz, jclass cls, jstring name, jstring sig, jboolean isstatic) {
    jmethodID m;
...
void hookedMethodCallback(const u4 *args, JValue *pResult, const Method *method, void *self) {
    ...
    ArrayObject *argTypes = dvmBoxMethodArgs(original,
                                             dvmIsStaticMethod(original) ? args : args + 1);
    JValue result;
...
jobject invoke(JNIEnv *env, jobject clazz, jobject obj, jobject args) {
    ...
    jmethodID m1 = env->GetStaticMethodID(fix, "invoke",
...
JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    if (isArt_) {
        ...
        if (dvm) {
            ...
            dvmCallMethod = (void (*)(void *self, const Method *method, Object *obj, void *pResult,
                                      ...)) (dlsym(dvm,
            ...
            dvmReleaseTrackedAlloc = (void (*)(Object *obj, void *self)) (dlsym(dvm,
                                                                                "dvmReleaseTrackedAlloc"));
            if (!dvmReleaseTrackedAlloc) {
                LOGD("dvmReleaseTrackedAlloc is null");
                return JNI_FALSE;
            }
            dvmUnboxPrimitive = (bool (*)(Object *value, ClassObject *returnType,
                                          void *pResult)) (dlsym(dvm,
                                                                 "_Z17dvmUnboxPrimitiveP6ObjectP11ClassObjectP6JValue"));
            if (!dvmUnboxPrimitive) {
                LOGD("dvmUnboxPrimitive is null");
                return JNI_FALSE;
            }
            dvmGetBoxedReturnType = (ClassObject *(*)(const Method *meth)) (dlsym(dvm,
                                                                                  "_Z21dvmGetBoxedReturnTypePK6Method"));
            if (!dvmGetBoxedReturnType) {
                LOGD("dvmGetBoxedReturnType is null");
                return JNI_FALSE;
            }
            dexProtoGetParameterCount = (size_t(*)(
            const DexProto *pProto))(dlsym(dvm, "_Z25dexProtoGetParameterCountPK8DexProto"));
            if (!dexProtoGetParameterCount) {
                LOGD("dexProtoGetParameterCount is null");
                return JNI_FALSE;
            }
            dvmAllocArrayByClass = (ArrayObject *(*)(ClassObject *arrayClass, size_t length,
                                                     int allocFlags)) (dlsym(dvm,
                                                                             "dvmAllocArrayByClass"));
            if (!dvmAllocArrayByClass) {
                LOGD("dvmAllocArrayByClass is null");
                return JNI_FALSE;
            }
            dvmFindSystemClass = (ClassObject *(*)(const char *descriptor)) (dlsym(dvm,
                                                                                   "_Z18dvmFindSystemClassPKc"));
            if (!dvmFindSystemClass) {
                LOGD("dvmFindSystemClass is null");
                return JNI_FALSE;
            }
            dvmFindPrimitiveClass = (ClassObject *(*)(char type)) (dlsym(dvm,
                                                                         "_Z21dvmFindPrimitiveClassc"));
            if (!dvmFindPrimitiveClass) {
                LOGD("dvmFindPrimitiveClass is null");
                return JNI_FALSE;
            }
            dvmBoxPrimitive = (void *(*)(JValue value, ClassObject *returnType)) (dlsym(dvm,
                                                                                        "_Z15dvmBoxPrimitive6JValueP11ClassObject"));
            if (!dvmBoxPrimitive) {
                LOGD("dvmBoxPrimitive is null");
                return JNI_FALSE;
            }
            jmethodID dexposedInvokeOriginalMethodNative = env->GetStaticMethodID(
                    env->FindClass(JAVA_CLASS), "invokeDavConstructor",
            ...
            dvmSetNativeFunc((void *) dexposedInvokeOriginalMethodNative,
                             (void *) invokeDavConstructor, NULL);
